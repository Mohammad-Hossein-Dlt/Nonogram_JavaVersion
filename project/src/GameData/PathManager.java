...
public class PathManager {
    ...
    public static ArrayList<File> folderList = GameDataFolder.list().length != 0 ? sortFolders() : new ArrayList<>();// A List To Sort Folders By Name
    public static void createNew(){
//      Create New GameData
        if (Files.notExists(GameDataFolder.toPath())) {
            GameDataFolder.mkdirs();
        }else if(GameDataFolder.listFiles().length == 0 ) {
    ...
    public static boolean lastPath() {
        if( GameHistory == null || isFinished(GameHistory) ) GameHistory = lastUnfinished();
        PathManager.load(GameHistory);
