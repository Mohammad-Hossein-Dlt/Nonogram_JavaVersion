...
public class Button extends JButton {
    ...
    public Button(int c, int r, boolean  continueGame) {
        ...
        if(continueGame) {
            if (Integer.parseInt(ReedData.GameConfig.get(2)) < 0) {
                this.setEnabled(false);
                this.setBackground(ThemeData.DisableButtonColor.Get());
            }
            if ( (ReedData.ShowingFalsePoints.contains(point) || ReedData.WrongClicked.contains(point)) ) {
                this.setEnabled(false);
                this.setBackground(ThemeData.FalseButtonColor.Get());
            }
            if (ReedData.CorrectClicked.contains(point)) {
                this.setEnabled(false);
