...
public class Button extends JButton {
    ...
    public Button(int c, int r, boolean  continueGame) {
        ...
        if(continueGame) {
            ...
            if (ReadData.CorrectClicked.contains(point)) {
                ...
                GameCore.columns.get("C" + this.column).remove(point);
                GameCore.rows.get("R" + this.row).remove(point);
                GameCore.truepoints.remove(point);
            }
        } else {
    ...
    public void click() {
        ...
        this.setEnabled(false);
        boolean check = GameCore.allpoint_.get(point);
        ...
        if (!GameCore.columns.get("C" + this.column).containsValue(true)) {
            PlaySound.play("Complete.wav");
            for (ArrayList<Integer> point : GameCore.columns.get("C" + this.column).keySet()) {
        ...
        if (!GameCore.rows.get("R" + this.row).containsValue(true)) {
            PlaySound.play("Complete.wav");
            for (ArrayList<Integer> point : GameCore.rows.get("R" + this.row).keySet()) {
                buttons.get(point).setBackground(ThemeData.FalseButtonColor.Get());
                buttons.get(point).setEnabled(false);
                SaveData.SaveRestOfFalsePoints(point);
            }
