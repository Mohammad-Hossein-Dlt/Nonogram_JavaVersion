...
//      -------------------------------
        mistakeNumber = continueGame ? Integer.parseInt(ReedData.GameConfig.get(2)) : Integer.parseInt(SettingsData.NumberOfChances);
        chancesLabel = new JLabel( mistakeNumber >= 0 ? Language.ChanceText.Get() : Language.NoMoreChanceText.Get());
        resultLabel = new JLabel(mistakeNumber >= 0 ? (continueGame && config.truepoints.isEmpty() ?  Language.WinGame.Get() : "Programed By Mh_Dlt" ) : Language.LoseGame.Get());
//      ------------------------------- Chances
//      ChancesLabel Config
